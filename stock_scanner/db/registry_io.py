...
_MONITORING_COLS = [
    ...
    "n_evaluated",
    "realized_win_rate",
    "realized_precision_at_threshold",
    "avg_predicted_prob",
    "feature_drift_flag",
    "alert_triggered",
]
...
def export_registry(conn: sqlite3.Connection, path: Path | None = None) -> Path:
    ...
    cur = conn.cursor()
    models = [
        dict(zip(_REGISTRY_COLS, row))
        for row in cur.execute(f"SELECT {','.join(_REGISTRY_COLS)} FROM model_registry").fetchall()
    ]
    decisions = [
        dict(zip(_DECISION_COLS, row))
        for row in cur.execute(
            f"SELECT {','.join(_DECISION_COLS)} FROM promotion_decisions"
        ).fetchall()
    ]
    monitoring = [
        dict(zip(_MONITORING_COLS, row))
        for row in cur.execute(
            f"SELECT {','.join(_MONITORING_COLS)} FROM live_monitoring"
        ).fetchall()
    ]
    path.write_text(
        json.dumps(
...
def import_registry(conn: sqlite3.Connection, path: Path | None = None) -> dict:
    ...
    conn.commit()
    return {
        "model_registry": n_models,
        "promotion_decisions": n_decisions,
        "live_monitoring": n_monitoring,
    }
