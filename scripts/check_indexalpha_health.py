...
def main(live: bool, ticker: str) -> int:
    ...
    if not key_configured:
        print("    INDEX_ALPHA_API_KEY not set in THIS environment (this is normal")
        print("    for a local shell / most CI jobs — only .github/workflows/")
        print("    indexalpha_live_check.yml wires the GitHub repo secret into env,")
        print("    deliberately, since spending quota should not happen by default).")
        print("    See [2] below for whether the integration has actually been")
        print("    proven to work somewhere, regardless of THIS environment's key.")
        print("    Streamlit Cloud's own secrets panel cannot be checked from here.")
